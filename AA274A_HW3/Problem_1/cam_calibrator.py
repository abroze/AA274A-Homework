...
class CameraCalibrator:
    ...
    def genCornerCoordinates(self, u_meas, v_meas):
        ...
        """
        ########## Code starts here ##########
        x_values = self.d_square*np.arange(0, self.n_corners_x, 1)
        y_values = self.d_square*np.arange(0, self.n_corners_y, 1)

        X, Y = np.meshgrid(x_values, y_values)

        Xg = np.array([X.flatten() for i in range(self.n_chessboards)])
        Yg = np.array([Y.flatten() for i in range(self.n_chessboards)])
    ...
    def estimateHomography(self, u_meas, v_meas, X, Y):  # Zhang Appendix A
        ...
        for i in range(self.n_corners_per_chessboard):
            L_i = np.array([[X[i], Y[i], 1, 0, 0, 0, -u_meas[i]*X[i], -u_meas[i]*Y[i], -u_meas[i]],
                            [0, 0, 0, X[i], Y[i], 1, -v_meas[i]*X[i], -v_meas[i]*Y[i], -v_meas[i]]])
            L[2*i:2*i+2,:] = L_i
    ...
    def getCameraIntrinsics(self, H):  # Zhang 3.1, Appendix B
        ...
        ########## Code starts here ##########
        def gen_v(H_b, i,j):
            return np.array([H_b[0,i]*H_b[0,j],
                             H_b[0,i]*H_b[1,j]+H_b[1,i]*H_b[0,j],
                             H_b[1,i]*H_b[1,j],
                             H_b[2,i]*H_b[0,j]+H_b[0,i]*H_b[2,j],
                             H_b[2,i]*H_b[1,j]+H_b[1,i]*H_b[2,j],
                             H_b[2,i]*H_b[2,j]])

        V = np.zeros((2*self.n_chessboards, 6))

        for i, H_b in enumerate(H):
            v_12 = gen_v(H_b, 0,1)
            v_11 = gen_v(H_b, 0,0)
            v_22 = gen_v(H_b, 1,1)
    ...
    def getExtrinsics(self, H, A):  # Zhang 3.1, Appendix C
        ...
        lamb = 1/np.linalg.norm(A_inv@h1)
    ...
    def transformWorld2NormImageUndist(
        ...
        M = np.vstack((X, Y, Z, np.ones_like(X)))
        mult = np.hstack((R, t.reshape(3,1))) @ M
        x = mult[0] / mult[2]
        y = mult[1] / mult[2]
    ...
    def transformWorld2PixImageUndist(
        ...
        M = np.vstack((X, Y, Z, np.ones_like(X)))
        mult = A @ np.hstack((R, t.reshape(3,1))) @ M
        u = mult[0] / mult[2]
        v = mult[1] / mult[2]
